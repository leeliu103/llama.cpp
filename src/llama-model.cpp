...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    if (arch == LLM_ARCH_OPENAI_MOE) {
        uint32_t n_fused_w = 0;
        uint32_t n_fused_b = 0;
        ...
        for (uint32_t il = 0; il < hparams.n_layer; ++il) {
            ...
            if (!layer.wqkv && layer.wq && layer.wk && layer.wv) {
                if (layer.wq->type != layer.wk->type || layer.wq->type != layer.wv->type) {
                    LLAMA_LOG_WARN("%s: layer %u: QKV type mismatch, skipping weight fusion\n", __func__, il);
                } else if (layer.wq->ne[0] != layer.wk->ne[0] || layer.wq->ne[0] != layer.wv->ne[0]) {
                    LLAMA_LOG_WARN("%s: layer %u: QKV inner-dimension mismatch, skipping weight fusion\n", __func__, il);
                } else {
                    ...
                    if (buft_q != buft_k || buft_q != buft_v) {
                        LLAMA_LOG_WARN("%s: layer %u: QKV buffer type mismatch, skipping weight fusion\n", __func__, il);
                    } else {
                        ...
                        if (ctx_qkv == nullptr) {
                            LLAMA_LOG_WARN("%s: layer %u: failed to create QKV context, skipping weight fusion\n", __func__, il);
                        } else {
                            ggml_tensor * wqkv = ggml_new_tensor_2d(ctx_qkv, wtype, n_embd, n_out_qkv);
                            if (wqkv == nullptr) {
                                ggml_free(ctx_qkv);
                                LLAMA_LOG_WARN("%s: layer %u: failed to create fused QKV tensor, skipping weight fusion\n", __func__, il);
                            } else {
                                ...
                                if (qkv_buf == nullptr) {
                                    ggml_free(ctx_qkv);
                                    LLAMA_LOG_WARN("%s: layer %u: failed to allocate fused QKV buffer, skipping weight fusion\n", __func__, il);
                                } else {
                                    ...
                                    if (n_fused_w == 1) {
                                        LLAMA_LOG_INFO(
                                            "%s: GPT-OSS fused QKV weights: Q[%lld,%lld] + K[%lld,%lld] + V[%lld,%lld] -> QKV[%lld,%lld] (%s)\n",
                                            __func__,
            ...
            if (!layer.bqkv && layer.bq && layer.bk && layer.bv) {
                if (layer.bq->type != layer.bk->type || layer.bq->type != layer.bv->type) {
                    LLAMA_LOG_WARN("%s: layer %u: QKV bias type mismatch, skipping bias fusion\n", __func__, il);
                } else {
                    ggml_backend_buffer_type_t buft_q = ggml_backend_buffer_get_type(layer.bq->buffer);
                    ggml_backend_buffer_type_t buft_k = ggml_backend_buffer_get_type(layer.bk->buffer);
                    ggml_backend_buffer_type_t buft_v = ggml_backend_buffer_get_type(layer.bv->buffer);

                    if (buft_q != buft_k || buft_q != buft_v) {
                        LLAMA_LOG_WARN("%s: layer %u: QKV bias buffer type mismatch, skipping bias fusion\n", __func__, il);
                    } else {
                        const int64_t n_out_q    = layer.bq->ne[0];
                        const int64_t n_out_k    = layer.bk->ne[0];
                        const int64_t n_out_v    = layer.bv->ne[0];
                        const int64_t n_out_qkv  = n_out_q + n_out_k + n_out_v;
                        const ggml_type btype    = layer.bq->type;
                        const size_t bq_nbytes   = ggml_nbytes(layer.bq);
                        const size_t bk_nbytes   = ggml_nbytes(layer.bk);
                        const size_t bv_nbytes   = ggml_nbytes(layer.bv);
                        const size_t bqkv_nbytes = bq_nbytes + bk_nbytes + bv_nbytes;

                        std::vector<uint8_t> bqkv_host(bqkv_nbytes);

                        ggml_backend_tensor_get(layer.bq, bqkv_host.data(), 0, bq_nbytes);
                        ggml_backend_tensor_get(layer.bk, bqkv_host.data() + bq_nbytes, 0, bk_nbytes);
                        ggml_backend_tensor_get(layer.bv, bqkv_host.data() + bq_nbytes + bk_nbytes, 0, bv_nbytes);

                        ggml_init_params ctx_bqkv_params = {
                            /*.mem_size   =*/ ggml_tensor_overhead(),
                            /*.mem_buffer =*/ nullptr,
                            /*.no_alloc   =*/ true,
                        };
                        ggml_context * ctx_bqkv = ggml_init(ctx_bqkv_params);
                        if (ctx_bqkv == nullptr) {
                            LLAMA_LOG_WARN("%s: layer %u: failed to create QKV bias context, skipping bias fusion\n", __func__, il);
                        } else {
                            ggml_tensor * bqkv = ggml_new_tensor_1d(ctx_bqkv, btype, n_out_qkv);
                            if (bqkv == nullptr) {
                                ggml_free(ctx_bqkv);
                                LLAMA_LOG_WARN("%s: layer %u: failed to create fused QKV bias tensor, skipping bias fusion\n", __func__, il);
                            } else {
                                ggml_set_name(bqkv, format("blk.%u.attn_qkv.bias", il).c_str());

                                const size_t bqkv_alloc_size = ggml_backend_buft_get_alloc_size(buft_q, bqkv);
                                ggml_backend_buffer_t bqkv_buf = ggml_backend_buft_alloc_buffer(buft_q, bqkv_alloc_size);
                                if (bqkv_buf == nullptr) {
                                    ggml_free(ctx_bqkv);
                                    LLAMA_LOG_WARN("%s: layer %u: failed to allocate fused QKV bias buffer, skipping bias fusion\n", __func__, il);
                                } else {
                                    ggml_backend_tensor_alloc(bqkv_buf, bqkv, ggml_backend_buffer_get_base(bqkv_buf));
                                    ggml_backend_tensor_set(bqkv, bqkv_host.data(), 0, bqkv_nbytes);
                                    ggml_backend_buffer_set_usage(bqkv_buf, GGML_BACKEND_BUFFER_USAGE_WEIGHTS);

                                    if (use_mlock && ggml_backend_buffer_is_host(bqkv_buf)) {
                                        pimpl->mlock_bufs.emplace_back(new llama_mlock);
                                        auto & mlock_buf = pimpl->mlock_bufs.back();
                                        mlock_buf->init   (ggml_backend_buffer_get_base(bqkv_buf));
                                        mlock_buf->grow_to(ggml_backend_buffer_get_size(bqkv_buf));
                                    }

                                    layer.bqkv = bqkv;
                                    pimpl->ctxs_bufs.emplace_back(ggml_context_ptr(ctx_bqkv), std::vector<ggml_backend_buffer_ptr>());
                                    pimpl->ctxs_bufs.back().second.emplace_back(bqkv_buf);
                                    tensors_by_name.emplace_back(ggml_get_name(bqkv), bqkv);
                                    n_fused_b++;
                                }
                            }
                        }
        ...
        if (n_fused_w > 0 || n_fused_b > 0) {
            LLAMA_LOG_INFO("%s: fused GPT-OSS QKV weights for %u layers and biases for %u layers\n", __func__, n_fused_w, n_fused_b);
        }
    }
#endif // GGML_USE_HIP
