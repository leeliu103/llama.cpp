...
struct gptoss_decode_aql {
    ...
    std::array<hsa_kernel_dispatch_packet_t, gptoss_config_120b.layer_count> packets{};
};

gptoss_decode_aql * gptoss_decode_aql_create(int multiprocessor_count, uint32_t layer_count) {
    if (layer_count == 0 || layer_count > gptoss_config_120b.layer_count) {
        return nullptr;
    }
    ...
    if (active_blocks <= 0 || multiprocessor_count <= 0) {
        LLAMA_LOG_ERROR("gptoss-aql: decode kernel has zero cooperative occupancy\n");
        return nullptr;
    }
    const uint32_t resident_blocks =
        static_cast<uint32_t>(active_blocks) * static_cast<uint32_t>(multiprocessor_count);
    const uint32_t grid_blocks =
        resident_blocks < gptoss_decode_grid_blocks ? resident_blocks : gptoss_decode_grid_blocks;
    ...
    aql->layer_count = layer_count;
    aql->queue       = llama_hip_aql_queue_create(marker_address, layer_count);
    ...
    aql->kernargs = static_cast<gptoss_decode_kernarg *>(
        llama_hip_aql_queue_alloc_kernarg(aql->queue, sizeof(gptoss_decode_kernarg) * layer_count));
    ...
    std::memset(aql->kernargs, 0, sizeof(gptoss_decode_kernarg) * layer_count);
    for (uint32_t il = 0; il < layer_count; ++il) {
        ...
        kernarg.block_count[0] = grid_blocks;
        ...
        packet.header = gptoss_packet_header(
            ...
            il + 1 == layer_count ? HSA_FENCE_SCOPE_SYSTEM : HSA_FENCE_SCOPE_AGENT);
        ...
        packet.grid_size_x = grid_blocks * gptoss_decode_block_x;
    ...
    gptoss_decode_grid_sync grid_sync{};
    grid_sync.num_workgroups = grid_blocks;
...
bool gptoss_decode_aql_launch(
    ...
    for (uint32_t il = 0; il < aql->layer_count; ++il) {
        aql->kernargs[il].params = params[il];
    }

    return llama_hip_aql_queue_submit_and_wait(aql->queue, aql->packets.data(), aql->layer_count);
}
