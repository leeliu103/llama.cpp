...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_mxfp4(
    ...
    const bool use_soa = soa_q_offset != 0;
    const uint8_t * x_q = (const uint8_t *) x;
    const uint8_t * x_e = use_soa ? x_q + soa_q_offset : nullptr;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        if (kbx < blocks_left) {
            ...
            const int aux_q4 = use_soa
                ? get_int_b1(x_q + ib*(QK_MXFP4/2), kqsx)
                : get_int_b1(((const block_mxfp4 *) x + ib)->qs, kqsx);
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * rows_per_warp) {
        ...
        if (kbxd < blocks_left) {
            ...
            d = use_soa
                ? ggml_cuda_e8m0_to_fp32(x_e[ib])*0.5f
                : ggml_cuda_e8m0_to_fp32(((const block_mxfp4 *) x + ib)->e)*0.5f;
        }
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += rows_per_warp * nwarps) {
        ...
        if (kbx < blocks_left) {
            const int ib = kbx0 + i * stride + kbx;
            const uint8_t * q = use_soa
                ? x_q + ib*(QK_MXFP4/2)
                : ((const block_mxfp4 *) x + ib)->qs;
        ...
        if (kbx % 2 == 0) {
            ...
            if (kbx < blocks_left) {
                ...
                if (use_soa) {
                    ...
                    const block_mxfp4 * bxi = (const block_mxfp4 *) x + ib;
                    e = bxi->e;
                    if (kbx + 1 < blocks_left) {
                        e |= ((uint32_t) (bxi + 1)->e << 8);
                    }
                }
            }
...
static __device__ __forceinline__ void mul_mat_q_process_tile(
    ...
    for (int kb0 = kb0_start; kb0 < kb0_stop; kb0 += blocks_per_iter) {
        load_tiles(x, tile_x, offset_x + kb0, tile_x_max_i, stride_row_x, soa_q_offset);
        {
...
static __global__ void mul_mat_q(
        ...
        const int ncols_max, const uint32_t soa_q_offset) {
...
#pragma unroll
        ...
        mul_mat_q_process_tile<type, mmq_x, need_check, fixup>
            (x, offset_x, y + offset_y, ids_dst_shared, dst + offset_dst, tmp_fixup, stride_row_x, ncols_y, stride_col_dst,
             tile_x_max_i, tile_y_max_j, 0, ncols_x/qk, soa_q_offset);
        return;
    }
...
#pragma unroll
        ...
        mul_mat_q_process_tile<type, mmq_x, need_check, fixup>
            (x, offset_x, y + offset_y, ids_dst_shared, dst + offset_dst, tmp_fixup, stride_row_x, ncols_y, stride_col_dst,
             tile_x_max_i, tile_y_max_j, kb0_start, kb0_stop, soa_q_offset);
...
struct mmq_args {
    ...
    bool use_stream_k; int64_t ncols_max; uint32_t soa_q_offset;
};
        ...
        if (args.nrows_x % mmq_y == 0) {
            ...
            mul_mat_q<type, mmq_x, need_check><<<block_nums_xy_tiling, block_dims, nbytes_shared, stream>>>
                (args.x, args.y, args.ids_dst, args.expert_bounds, args.dst, nullptr,
                 ...
                 args.ncols_max, args.soa_q_offset);
        } else {
