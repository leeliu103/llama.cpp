...
static void mul_mat_vec_q_switch_fusion(
        ...
        const uint32_t ids_stride,
        const uint32_t soa_q_offset, cudaStream_t stream) {
        ...
        if (has_fusion) {
            mul_mat_vec_q<type, c_ncols_dst, true, small_k><<<block_nums, block_dims, nbytes_shared, stream>>>
                (vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
                 ...
                 sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, soa_q_offset);
            return;
        }
    }
...
static void mul_mat_vec_q_moe_launch(
    ...
    mul_mat_vec_q_moe<type, rows_per_block><<<block_nums, block_dims, 0, stream>>>(
        ...
        ncols_dst, ids_stride, soa_q_offset);
}
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    if (has_ids && ncols_dst > 1) {
        ...
        mul_mat_vec_q_moe_launch<type>(
            ...
            stride_row_x, stride_col_y, stride_col_dst,
            stride_channel_x, stride_channel_y, stride_channel_dst,
            ncols_dst, ids_stride, soa_q_offset, warp_size, nchannels_dst, stream);
        return;
    }
    ...
    switch (ncols_dst) {
        case 1: {
            ...
            if (use_small_k) {
                ...
                mul_mat_vec_q_switch_fusion<type, c_ncols_dst, true>(
                    ...
                    stride_sample_x, stride_sample_y, stride_sample_dst, dims.first, dims.second, 0, ids_stride, soa_q_offset,
                    stream);
            } else {
                std::pair<dim3, dim3> dims = calc_launch_params<type>(c_ncols_dst, nrows_x, nchannels_dst,
        ...
        case 2: {
            ...
            mul_mat_vec_q_switch_fusion<type, c_ncols_dst>(vx, vy, ids, fusion, dst, ncols_x, nchannels_y_fd, stride_row_x, stride_col_y, stride_col_dst,
                 ...
                 dims.first, dims.second, 0, ids_stride, soa_q_offset, stream);
        } break;
        case 3: {
        ...
        case GGML_TYPE_Q1_0:
            mul_mat_vec_q_switch_ncols_dst<GGML_TYPE_Q1_0>
                (vx, vy, ids, fusion, dst, ncols_x, nrows_x, ncols_dst, stride_row_x, stride_col_y, stride_col_dst,
                 ...
                 nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, soa_q_offset, stream);
            break;
        case GGML_TYPE_Q4_0:
...
void ggml_cuda_mul_mat_vec_q(
    ...
    const uint64_t soa_q_offset_u64 = src0->type == GGML_TYPE_MXFP4
        ? ((uint64_t) ggml_nelements(src0) / ggml_blck_size(src0->type)) * (QK_MXFP4 / 2)
        : 0;
    GGML_ASSERT(soa_q_offset_u64 <= UINT32_MAX);
    const uint32_t soa_q_offset = (uint32_t) soa_q_offset_u64;

    mul_mat_vec_q_switch_type(
        src0->data, src0->type, src1_q8_1.get(), ids_d, fusion_local, dst_d, ne00,
        ne01,              ncols_dst,     s01, stride_col_y,     stride_col_dst,
        ne02, nchannels_y, nchannels_dst, s02, stride_channel_y, stride_channel_dst,
        ne03,              ne3,           s03, s13,              s3,               ids_stride, soa_q_offset, stream);
}
