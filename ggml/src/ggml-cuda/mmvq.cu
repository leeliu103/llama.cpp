...
static __device__ __forceinline__ float vec_dot_q4_K_q8_1_x4(
        const block_q4_K * __restrict__ bq4_K_row,
        const block_q8_1_x4 * __restrict__ yx4,
        const int kby, const int kbx, const int subblock_pair) {
    const int subblock0 = 2 * subblock_pair;
    const int subblock1 = subblock0 + 1;
    const q4_k_q8_1_x4_rhs rhs0 = load_q4_K_q8_1_x4_rhs(yx4, kby, subblock0);
    const q4_k_q8_1_x4_rhs rhs1 = load_q4_K_q8_1_x4_rhs(yx4, kby, subblock1);
    return vec_dot_q4_K_q8_1_x4_with_rhs_pair(bq4_K_row + kbx, rhs0, rhs1, subblock_pair);
}

static __device__ __forceinline__ float2 vec_dot_q4_K_q8_1_x4_x2(
        const block_q4_K * __restrict__ bq4_K_x_row,
        const block_q4_K * __restrict__ bq4_K_gate_row,
        const block_q8_1_x4 * __restrict__ yx4,
        const int kby, const int kbx, const int subblock_pair) {
    const int subblock0 = 2 * subblock_pair;
    const int subblock1 = subblock0 + 1;
    ...
    const q4_k_q8_1_x4_rhs rhs1 = load_q4_K_q8_1_x4_rhs(yx4, kby, subblock1);
    return make_float2(
        vec_dot_q4_K_q8_1_x4_with_rhs_pair(bq4_K_x_row + kbx, rhs0, rhs1, subblock_pair),
        vec_dot_q4_K_q8_1_x4_with_rhs_pair(bq4_K_gate_row + kbx, rhs0, rhs1, subblock_pair));
}
...
static __device__ __forceinline__ float apply_mmvq_q8_1_x4_glu(
...
template <bool has_fusion, bool has_gate_fusion>
static __device__ __forceinline__ float finalize_mmvq_q8_1_x4_result(
        float sum, float gate_sum, const ggml_cuda_mm_fusion_args_device & fusion,
        const uint32_t bias_base, const int row) {
    float result = sum;

    if constexpr (has_fusion) {
        if (fusion.x_bias != nullptr) {
            result += ((const float *) fusion.x_bias)[bias_base + row];
        }
        if constexpr (has_gate_fusion) {
            float gate_value = gate_sum;
            if (fusion.gate_bias != nullptr) {
                gate_value += ((const float *) fusion.gate_bias)[bias_base + row];
            }
            return apply_mmvq_q8_1_x4_glu(result, gate_value, fusion.glu_op);
        }
        GGML_UNUSED(gate_sum);
    } else {
        GGML_UNUSED_VARS(gate_sum, fusion, bias_base, row);
    }

    return result;
}

static inline bool mmvq_q8_1_x4_has_fusion(const ggml_cuda_mm_fusion_args_device & fusion) {
...
static __device__ __forceinline__ void accumulate_q4_K_q8_1_x4_iter(
        const block_q4_K * __restrict__ vx_row,
        const block_q4_K * __restrict__ vgate_row,
...
static __global__ void mul_mat_vec_q4_K_q8_1_x4(
    ...
    const block_q4_K * vx_row = ((const block_q4_K *) vx) + kbx_offset;
    const block_q4_K * vgate_row = nullptr;
    if constexpr (has_gate_fusion) {
...
#pragma unroll
    ...
    if (threadIdx.y == 0 && threadIdx.x == 0) {
        ...
        const uint32_t bias_base = sample_dst*stride_sample_dst + channel_bias*stride_channel_dst;
        dst[dst_base + row0] = finalize_mmvq_q8_1_x4_result<has_fusion, has_gate_fusion>(tmp, tmp_gate, fusion, bias_base, row0);
    }
...
static __global__ void mul_mat_vec_q4_K_q8_1_x4_rowpair_gate(
    ...
    if (threadIdx.x == 0) {
        const uint32_t dst_base = sample_dst*stride_sample_dst + channel_dst*stride_channel_dst;
        dst[dst_base + row0] = finalize_mmvq_q8_1_x4_result<true, true>(tmp0, tmp0_gate, fusion, dst_base, row0);

        if (has_row1) {
            dst[dst_base + row1] = finalize_mmvq_q8_1_x4_result<true, true>(tmp1, tmp1_gate, fusion, dst_base, row1);
        }
    }
}
...
static void dispatch_mul_mat_vec_q4_K_q8_1_x4(
        ...
        const uint32_t ids_stride, cudaStream_t stream) {
    const bool has_fusion = mmvq_q8_1_x4_has_fusion(fusion);
...
static __device__ __forceinline__ float vec_dot_q6_K_q8_1_x4(
        const block_q6_K * __restrict__ bq6_K_row, const block_q8_1_x4 * __restrict__ yx4,
        const int kby, const int kbx, const int iqs) {
    const q6_k_q8_1_x4_rhs rhs = load_q6_K_q8_1_x4_rhs(yx4, kby, iqs);
    return vec_dot_q6_K_q8_1_x4_with_rhs(bq6_K_row + kbx, rhs, iqs);
}

static __device__ __forceinline__ float2 vec_dot_q6_K_q8_1_x4_x2(
        const block_q6_K * __restrict__ bq6_K_x_row, const block_q6_K * __restrict__ bq6_K_gate_row,
        const block_q8_1_x4 * __restrict__ yx4, const int kby, const int kbx, const int iqs) {
    const q6_k_q8_1_x4_rhs rhs = load_q6_K_q8_1_x4_rhs(yx4, kby, iqs);

    return make_float2(
        vec_dot_q6_K_q8_1_x4_with_rhs(bq6_K_x_row + kbx, rhs, iqs),
        vec_dot_q6_K_q8_1_x4_with_rhs(bq6_K_gate_row + kbx, rhs, iqs));
}

template <bool has_gate_fusion>
static __device__ __forceinline__ void accumulate_q6_K_q8_1_x4_iter(
        const block_q6_K * __restrict__ vx_row, const block_q6_K * __restrict__ vgate_row,
...
static __global__ void mul_mat_vec_q6_K_q8_1_x4(
    ...
    const block_q8_1_x4 * y_row = ((const block_q8_1_x4 *) vy) + sample_y*stride_sample_y + channel_y*stride_channel_y;
    const block_q6_K * vx_row = ((const block_q6_K *) vx) + kbx_offset;
    const block_q6_K * vgate_row = nullptr;
    if constexpr (has_gate_fusion) {
